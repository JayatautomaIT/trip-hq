...
export function IdeasView() {
  ...
  return (
    <div>
      <p className="page-sub">Pitch anything — bars, activities, restaurants. Everyone votes; the organizer pins the winners.</p>

      {!open && <button className="btn primary" onClick={() => setOpen(true)}>+ Add idea</button>}
      {open && (
        <div className="card">
          <div className="grid cols-2">
            <div><label>Idea</label><input autoFocus value={f.title} onChange={(e) => setF({ ...f, title: e.target.value })} placeholder="Go-karting Saturday" /></div>
        ...
        {ideas.map((i) => (
          <div key={i.id} className={`card ${i.pinned ? 'pinned-card' : ''}`}>
            <div className="row" style={{ alignItems: 'flex-start' }}>
              <button className={`vote-btn ${i.voted ? 'voted' : ''}`} onClick={() => jpost(`/api/ideas/${i.id}/vote`).then(load)}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {i.url && <div className="small" style={{ marginTop: 4 }}><a href={i.url} target="_blank" rel="noreferrer">{i.url}</a></div>}
            ...
            {isAdmin && (
              <div className="row" style={{ marginTop: 10 }}>
                ...
                <button className="btn sm ghost" onClick={() => jpatch('/api/ideas', { id: i.id, status: i.status === 'decided' ? 'open' : 'decided' }).then(load)}>
                  {i.status === 'decided' ? 'Un-decide' : 'Decided'}
                </button>
                ...
                <button className="btn sm danger" onClick={() => jdel('/api/ideas', { id: i.id }).then(load)}>✕</button>
              </div>
            )}
          </div>
        ))}
